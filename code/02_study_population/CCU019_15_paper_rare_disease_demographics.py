...
# MAGIC (cloned from CCU013 by Honghan ) 
# MAGIC **Description** 
# MAGIC 
...
# MAGIC 4. Cleaning
# MAGIC 5. Create output table `ccu019_rare_disease_demographics_cohort` 1 row per patient
# MAGIC 
# MAGIC **Project(s)** CCU019
# MAGIC  
# MAGIC **Author(s)** Honghan Wu (reused codes from CCU013 Chris Tomlinson)
# MAGIC  
# MAGIC **Reviewer(s)** 
# MAGIC  
# MAGIC **Date last updated** 2021-08-27
# MAGIC  
# MAGIC **Date last reviewed** *NA*
# MAGIC  
# MAGIC **Date last run** 2021-08-27
# MAGIC  
# MAGIC **Data input**  
...
# MAGIC 
# MAGIC **Data output**  
# MAGIC * `dars_nic_391419_j3w9t_collab.ccu019_rare_disease_demographics_cohort` 1 row per patient
# MAGIC 
...
# Check coverage
events = spark.table('dars_nic_391419_j3w9t_collab.ccu019_paper_cohort_rare_disease')
...
events = spark.table('dars_nic_391419_j3w9t_collab.ccu019_paper_cohort_rare_disease')\
        .withColumnRenamed("date", "date_first")
...
# MAGIC Calculate age at time of first __Rare disease__ medical coding (`date_first` in `ccu019_paper_cohort_rare_disease`)
...
SELECT * FROM (SELECT
  ...
  MAX(DATE) as date
FROM
  dars_nic_391419_j3w9t_collab.gdppr_dars_nic_391419_j3w9t_archive as gdppr WHERE ProductionDate == "{production_date}"
GROUP BY
...
LEFT JOIN
  (SELECT 
    ...
    DATE as date,
    LSOA as lsoa 
  FROM dars_nic_391419_j3w9t_collab.gdppr_dars_nic_391419_j3w9t_archive as gdppr WHERE ProductionDate == "{production_date}") as tab2
ON
...
FROM
   global_temp.ccu013_dp_gdppr_tmp
WHERE
...
# Test no duplicates
if cohort.count() > cohort.dropDuplicates(['person_id_deid']).count():
#     raise ValueError('Cohort contains duplicate ids when should be mutually exclusive')
  cohort = cohort.dropDuplicates(['person_id_deid'])
  print('cohort count after deduplication', cohort.count())
...
cohort.createOrReplaceGlobalTempView("ccu019_rare_disease_demographics_cohort")
drop_table("ccu019_rare_disease_demographics_cohort") 
create_table("ccu019_rare_disease_demographics_cohort") 

# COMMAND ----------

# MAGIC %sql
# MAGIC OPTIMIZE dars_nic_391419_j3w9t_collab.ccu019_rare_disease_demographics_cohort ZORDER BY person_id_deid

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM dars_nic_391419_j3w9t_collab.ccu019_rare_disease_demographics_cohort limit 10

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT COUNT(*), COUNT(DISTINCT person_id_deid) FROM dars_nic_391419_j3w9t_collab.ccu019_rare_disease_demographics_cohort
...
# MAGIC %md
# MAGIC # ISSUE
# MAGIC Other Ethnic Groups is not correctly stored under `ethnicity`  
# MAGIC It looks like `Other Ethnic Gr oups` but `regexp_replace` doesn't work and `== 'Other Ethnic Gr oups'` = `FALSE`
# MAGIC 
# MAGIC Failed:
# MAGIC * `cohort.withColumn('ethnicity', regexp_replace(col('ethnicity'), 'Other Ethnic Gr oups', 'Other Ethnic Groups')).select('ethnicity').distinct().show()`
# MAGIC * `cohort.withColumn('ethnicity', regexp_replace(col("ethnicity"), " ", "")).select('ethnicity').distinct().show()`
# MAGIC 
# MAGIC **Workaround**  
# MAGIC `count(when(col('ethnicity').like('Other%'), True))`

# COMMAND ----------

cohort.groupBy('ethnic_group').count().orderBy('count').show()
